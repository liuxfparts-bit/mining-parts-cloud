...
export default function SupplierProfileClient({ supplier }: { supplier: Props }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg border p-6 space-y-4">
      ...
      {readonly && <div className="bg-yellow-50 text-yellow-800 px-4 py-2 rounded text-sm">企业已认证，仅可修改联系方式。</div>}
      <div className="grid grid-cols-2 gap-4">
        <div><label className="block text-sm mb-1">企业名称</label><input name="name" defaultValue={supplier.name} readOnly={readonly} className={readonly ? ro : f} /></div>
        ...
        <div><label className="block text-sm mb-1">省</label><input name="province" defaultValue={supplier.province || ""} readOnly={readonly} className={readonly ? ro : f} /></div>
        <div><label className="block text-sm mb-1">市</label><input name="city" defaultValue={supplier.city || ""} readOnly={readonly} className={readonly ? ro : f} /></div>
        <div><label className="block text-sm mb-1">地址</label><input name="address" defaultValue={supplier.address || ""} readOnly={readonly} className={readonly ? ro : f} /></div>
        <div><label className="block text-sm mb-1">主营业务</label><input name="mainBusiness" defaultValue={supplier.mainBusiness} readOnly={readonly} className={readonly ? ro : f} /></div>
        <div><label className="block text-sm mb-1">主营品牌</label><input name="mainBrands" defaultValue={supplier.mainBrands || ""} readOnly={readonly} className={readonly ? ro : f} /></div>
        <div><label className="block text-sm mb-1">主营设备</label><input name="mainEquipment" defaultValue={supplier.mainEquipment || ""} readOnly={readonly} className={readonly ? ro : f} /></div>
      </div>
